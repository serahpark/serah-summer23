...
for email in body_text:
    ...
    emails.append([token.text for token in email if not token.is_stop and not token.is_punct and not token.like_num and len(token.text) > 2])
    # else:
...
print('dumped at ' + config['tokens_pickle'])